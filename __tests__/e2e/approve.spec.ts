...
test('审核申请流程测试', async ({ page, context }) => {
  // 生成随机邮箱以避免重复注册
  ...
  try {
    ...
    await petCards.first().waitFor({ timeout: 15000 });
    ...
    // 提交领养申请
    try {
      const adoptButtons = applicantPage.locator('button').filter({ hasText: '申请领养' });
      const adoptButtonCount = await adoptButtons.count();
      console.log('找到的申请领养按钮数量:', adoptButtonCount);
      
      if (adoptButtonCount > 0) {
        await adoptButtons.first().click();
        
        // 等待申请表单加载
        await applicantPage.waitForSelector('textarea', { timeout: 15000 });
        await applicantPage.fill('textarea', '我非常喜欢这只宠物，有足够的时间和精力照顾它。');
        
        // 提交申请
        const submitButtons = applicantPage.locator('button').filter({ hasText: '提交' });
        if (await submitButtons.count() > 0) {
          await submitButtons.first().click();
        }
      }
    } catch (error) {
      console.log('提交申请时出错:', error);
    }
    ...
    if (await viewButtons.count() > 0) {
      ...
      // 在详情页面点击通过申请按钮
      const approveButtons = page.locator('button').filter({ hasText: '通过申请' });
      if (await approveButtons.count() > 0) {
        await approveButtons.first().click();
        console.log('点击了通过申请按钮');
      }
    }
